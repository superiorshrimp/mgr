...
# RESULTS_PATH = 'results/selection/maxdist/'
...
def main():
    ...
    i = 0
    for d_key in sorted(y_values.keys()):
        plt.plot(avg_y_values[d_key], color=colors[i], label='delay = ' + str(d_key) + "ms", alpha=0.7)
        i += 1
    ...
    # x_vals = list(coef_values)
    #
    # ticks = list(range(len(x_vals)))
    # labels = sorted(x_vals)
    #
    # plt.xticks(
    #     ticks=ticks,
    #     labels=[label if i % 2 == 0 else "" for i, label in enumerate(labels)]
    # )
    #
    # plt.ylabel('average best fitness')
    # plt.xlabel('selection_method_coef')

...
class GeneticIslandAlgorithm:
    ...
    def add_new_individuals(self):
        if self.evaluations != 0 and self.evaluations == self.last_migration_evaluation_number: # TODO: revert (temp)
            ...
            #     i.energy = 0
            new_individuals = self.accept_all_individuals(new_individuals)
            # new_individuals = self.accept_avg_individuals(new_individuals)
            # new_individuals = self.accept_best_individuals(new_individuals)
    ...
    def accept_all_individuals(self, new_individuals):
    ...
    def accept_best_individuals(self, new_individuals):
        accepted_individuals = []
        best = self.last_best
        for new_individual in new_individuals:
            if best < new_individual.fitness: continue
            flag = False
            for individual in self.solutions:
                if new_individual.x == individual.x:
                    flag = True
    ...
    def accept_avg_individuals(self, new_individuals):
        accepted_individuals = []
